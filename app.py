...
def backtesting_normal(close, señales, capital_inicial=10_000):
    ...
    df_equity["Buy & Hold"] = unidades_bh * close_vals[1:]
...
def render_backtesting_normal(ticker, capital_inicial):
    ...
    cb3.metric("Buy & Hold Final", f"${bh_final:,.2f}", delta=f"{bh_pct:+.1f}%")
    cb4.metric("Ventaja vs B&H", f"${ganancia_total - bh_ganancia:+,.2f}")
...
modo = st.radio("Según tu criterio, como consideras que se esta comportado el mercado financiero:",
    ...
    if st.session_state.modo == "Normal":
        ...
        for i, t in enumerate(tickers_actuales):
            with cols_cap[i % 4]:
                ...
                nuevo_val = st.number_input(f"Capital — {t} (USD)",
        ...
        for i, t in enumerate(tickers_actuales):
            with cols_cap[i % 4]:
                ...
                nuevo_val = st.number_input(f"Capital DCA — {t} (USD)",
